...
contains
    ...
    recursive subroutine dfs(this, node_id)
        class(Graph)::this
        character(len=*), intent(in)::node_id
        integer::i, j
        integer,allocatable::adjacency_nodes_index(:)
        ...
        if (this%nodes(i)%node_color == 1) then
            return
        end if
        ...
        if(op_result) then
            do j = 1, size(adjacency_nodes_index)
                call this%dfs(this%nodes(adjacency_nodes_index(j))%node_id)
            end do
        end if
    ...
    subroutine is_connected_graph(this, op_result)
        ...
        do i = 1, size(this%nodes)
            if (this%nodes(i)%is_present) then
                call this%dfs(this%nodes(i)%node_id)
                exit
            end if
        end do
